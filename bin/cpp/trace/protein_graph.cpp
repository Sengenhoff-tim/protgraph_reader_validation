...
std::ofstream trace("cpp_trace.log");
...
void ProteinGraph::write_intervals_compact(const std::string& path) const {
    ...
    for (uint32_t node = 0; node < this->N; ++node) {
        file << "node_" << node << ": ";
        
        bool first = true;
        for (int j = 0; j < 2 * this->PDB; j += 2) {
            int64_t lower = this->pdbs[node * 2 * this->PDB + j];
            int64_t upper = this->pdbs[node * 2 * this->PDB + j + 1];

            if (lower == INT64_MAX || upper == INT64_MAX) {
                continue;
            }
...
std::string ProteinGraph::tvs_traverse_varcount_naive(int64_t lower, int64_t upper, uint8_t max_vars, std::ofstream& trace) {
    ...
    for (uint32_t i = 0; i < this->N-1; i++) {
        ...
        for (uint32_t j = 0; j < tv_vals[i].size(); j++) {   
            ...
            for (uint32_t k = e_b; k < e_e; k++) {
                ...
                bool overlap = this->overlapping_interval(
                    target_node,
                    new_lower,
                    new_upper
                );

                bool accepted = (current_var_count <= max_vars) && overlap;
                ...
                trace
                    ...
                    << new_lower << " "
                    << new_upper << " "
                    << static_cast<int>(current_var_count) << " "
                    << static_cast<int>(max_vars) << " " 
                    << overlap << " "
                    << accepted
                    << "\n";
