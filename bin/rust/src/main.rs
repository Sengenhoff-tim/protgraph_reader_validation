...
fn main() -> Result<()> {
    ...
    let channel_dedup_out_size = config.cli.ch_proc_out_size.unwrap_or(config.cli.avail_processors*2);
    ...
    dedup_bin_files(tmp_files, avail_processors, &outdir_path, zip, channel_dedup_in_size, channel_dedup_out_size)?;
